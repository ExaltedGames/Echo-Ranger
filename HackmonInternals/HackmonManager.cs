...
public static class HackmonManager
{
	...
	private static readonly Dictionary<string, StatusInitializer> _statusMap = new();
	...
	public static void LoadAllData()
	{
		...
		foreach (var move in moves)
		{
			...
			catch (ArgumentException e)
			{
				Console.WriteLine(e);
				Console.WriteLine(
					$"Duplicate move ID for move {move.Name}. ID {move.Id} already belongs to {MoveRegistry[move.Id].Name}."
				);
		...
		var elementJson = File.ReadAllText(
			Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Data/ElementInteractions.json")
		);
	...
	public static Status InstanceStatus(string status, HackmonInstance unit, int stacks)
	{
		if (!_statusMap.ContainsKey(status))
			throw new Exception($"No such status currently loaded: {status}");
